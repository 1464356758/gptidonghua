...
public final class Checks extends Instrumentation {
    ...
    @Override public void onStart(){Bundle result=new Bundle();try{
        ...
        String received="";for(int i=0;i<100;i++){received=tree(root());if(received.contains("Share received:")||received.contains("Share failed:"))break;Thread.sleep(300);}File[] sharedFiles=dir.listFiles();Arrays.sort(sharedFiles,Comparator.comparing(File::getName));File sharedFile=sharedFiles[sharedFiles.length-1];check(received.contains("Share received: "+sha(sharedFile))&&received.contains("Bytes: "+sharedFile.length()),"external app received exact ZIP through read grant");check(received.contains("Receiver UID:")&&!received.contains("Receiver UID: "+android.os.Process.myUid()),"share receiver uses independent app UID");shot("android-share-received.png");sendKeyDownUpSync(KeyEvent.KEYCODE_BACK);waitJS("!document.getElementById('generate').disabled");check(true,"share return");
