...
public final class Checks extends Instrumentation {
    ...
    @Override public void onStart(){Bundle result=new Bundle();try{
        ...
        js("document.getElementById('share').click()");boolean chooser=false;for(int i=0;i<100;i++){AccessibilityNodeInfo r=root();String t=tree(r);if(t.contains("isn't responding"))throw new AssertionError("System ANR during share");String pkg=r==null?"":String.valueOf(r.getPackageName());if(pkg.equals("android")||pkg.contains("intentresolver")){chooser=true;break;}Thread.sleep(300);}check(chooser,"rendered share chooser");Thread.sleep(1000);shot("android-share.png");sendKeyDownUpSync(KeyEvent.KEYCODE_BACK);waitJS("!document.getElementById('generate').disabled");check(true,"share chooser return");
